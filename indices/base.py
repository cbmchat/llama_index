...
def refresh_single_document(
    ...
    update_ref_doc_func,
    get_document_hash_func,
    insert_func,
    update_kwargs: dict
) -> bool:
    existing_doc_hash = get_document_hash_func(document.get_doc_id())
    if existing_doc_hash != document.hash:
        update_ref_doc_func(document, **update_kwargs)
        return True
    elif existing_doc_hash is None:
        insert_func(document, **update_kwargs)
        return True
    return False
