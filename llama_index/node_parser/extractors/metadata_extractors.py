...
DEFAULT_TITLE_NODE_TEMPLATE = """\
上下文：{context_str}。请提供一个总结上下文中所有独特实体、标题或主题的标题。标题："""
...
DEFAULT_TITLE_COMBINE_TEMPLATE = """\
{context_str}。基于上述候选标题和内容，这份文档的综合标题是什么？标题："""
...
#                 ),
                PromptTemplate(
                    template=f"""\
{{context_str}}。为这份文档提供{self.keywords}个唯一关键词。
以逗号分隔的格式。关键词："""
                ),
                context_str=cast(TextNode, node).text,
            )
...
基于上下文信息，生成{num_questions}个问题，这些问题可以提供特定答案，而这些答案在其他地方可能不容易找到。

也可以提供周围上下文的高层摘要。尝试使用这些摘要来生成上下文能够回答的更好的问题。
